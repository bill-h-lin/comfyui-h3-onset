...
`minimax_h3_ref2va_pruned_int8`, 8 steps, 124 frames, 1344×768, local. **`selftest.py` re-checks
...
Landing is **slightly early by a constant** — see Scope, where the size and the reason are set out.
...
- 🔴 **We do NOT command *what*.** In the bronze test above the placed event was heard as a
  *click*, not a bell — on a scene depicting a struck bell. The node places an event and makes it
...
- **Use a speech-free carrier.** A carrier containing speech reproduces transients *before* its own
  voice onset and **goes deaf after it** (+39/+43/+39 dB, then −5.2/+0.4/+0.7/+0.6) — **and it
...
- ⭐ **The lead is a fixed offset, not a fraction of the commanded time — so your carrier does not
  ...
  carrier cross-correlates **~2× better** than one resampled to the render's length, on **4 of 4**
...
Independent of both nodes, and the single most useful thing we learned:

There are **two** ways to make H3 generate impact sounds you never asked for. Both are in the
prompt, and one of them is in the *picture* line, which nobody expects.
...
⭐ **It is not the sound-word** — `chatter` and `jolt` score identically. ⭐ **And it is not depicting
an impact** — the third row depicts one and sits on the floor. **It is the repetition.** Deleting
...
not choose.** Same object, same room, same seeds, clean carrier, nothing placed — and the *picture*
...
⇒ **Depict the impacts in the picture; never name them in the soundscape.** The picture may show all
